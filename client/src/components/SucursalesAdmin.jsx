...
import { etiquetaModulos } from '../utils/modulos'

const OPCIONES_MODULO = [
  { value: 'FIT_MARKET', label: 'Fit Market' },
  { value: 'GYM', label: 'Infinity Academia' },
]
...
function SucursalesAdmin({
  ...
  const { user } = useAuth()
  const reloadMarket = useSucursalStore((state) => state.load)
  const reloadGym = useGymSucursalStore((state) => state.load)
  ...
  const reloadStores = () =>
    Promise.all([reloadMarket({ silent: true }), reloadGym({ silent: true })])
  ...
  const toggleModulo = (value) => {
    setModulos((actuales) =>
      actuales.includes(value)
        ? actuales.filter((item) => item !== value)
        : [...actuales, value],
    )
  }
  ...
  const openEdit = (local) => {
    ...
    setModulos(local.modulos?.length ? local.modulos : defaultModulos)
    setOpen(true)
  }
  ...
  const guardar = async (event) => {
    ...
    if (modulos.length === 0) {
      Swal.fire({ icon: 'warning', title: 'Elegí al menos un módulo' })
      return
    }
    try {
      if (editing) {
        await sucursalesApi.update(editing.id, { nombre, modulos })
      } else {
        await sucursalesApi.create({ nombre, modulos })
      }
      setOpen(false)
      await Promise.all([load(), reloadStores()])
    } catch (err) {
            ...
            <tbody>
              {locales.map((local) => (
                <tr key={local.id} className="border-t border-slate-100">
                  ...
                  <td className="px-4 py-3 text-slate-700">{etiquetaModulos(local.modulos)}</td>
                  <td className="px-4 py-3">
                    <span
                      className={
                        local.activa
                  ...
                  <td className="px-4 py-3 text-right">
                    <button
                      ...
                      aria-label={`Editar ${local.nombre}`}
                    >
          ...
          <form onSubmit={guardar} className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
            ...
            <label className="mt-4 block text-xs text-slate-500">
              ...
              <input
                ...
                placeholder="Infinity Centro"
                required
              />
            </label>
            <fieldset className="mt-4">
              <legend className="text-xs text-slate-500">Módulos</legend>
              <div className="mt-2 grid gap-2">
                {OPCIONES_MODULO.map((opcion) => (
                  <label
                    key={opcion.value}
                  ...
                  >
                    <input
                      type="checkbox"
                      checked={modulos.includes(opcion.value)}
                      onChange={() => toggleModulo(opcion.value)}
                    />
                    {opcion.label}
                  </label>
                ))}
